...
class FinishResilientOptimistic extends FinishResilient implements CustomSerialization {
    ...
    public static final class OptimisticRemoteState extends FinishResilient implements x10.io.Unserializable {
        ...
        public def notifyTerminationAndGetMap(t:Task) {
            ...
            //NOLOG if (verbose>=1) {
            //NOLOG    if (map == null)
            //NOLOG        debug("<<<< Remote(id="+id+").notifyTerminationAndGetMap returning null, localCount = " + lc);
            //NOLOG    else {
            //NOLOG        val s = new x10.util.StringBuilder();
            //NOLOG        for (e in map.entries()) {
            //NOLOG            s.add(e.getKey()+" = "+e.getValue()+", ");
            //NOLOG        }
            //NOLOG        debug("<<<< Remote(id="+id+").notifyTerminationAndGetMap returning, MAP = {"+s.toString()+"} ");
            //NOLOG    }
            //NOLOG }
            //NOLOG if (verbose>=3) dump();
    ...
    public static final class OptimisticMasterState extends FinishMasterState implements x10.io.Unserializable {
        ...
        def this(id:Id, parent:FinishState, src:Place, kind:Int) {
            ...
            if (parent instanceof FinishResilientOptimistic) {
                ...
                //NOLOG if (verbose>=1) debug("FinishResilientOptimistic(id="+id+") foreign parent found of type " + parent);
                parentId = UNASSIGNED;
            }
        }
    ...
    static def getCountingRequests(newDead:HashSet[Int],
      ...
      //NOLOG if (verbose >=1 ) {
      //NOLOG     val s = new x10.util.StringBuilder();
      //NOLOG     if (resolveReqs.size() > 0) {
      //NOLOG         for (e in resolveReqs.entries()) {
      //NOLOG             val pl = e.getKey();
      //NOLOG             val reqs = e.getValue();
      //NOLOG       val bkps = reqs.countBackups;
      //NOLOG       val recvs = reqs.countReceived;
      //NOLOG       s.add("\nRecovery requests:\n");
      //NOLOG       s.add("   To place: " + pl + "\n");
      //NOLOG       if (bkps.size() > 0) {
      //NOLOG           s.add("  countBackups:{ ");
      //NOLOG           for (b in bkps.entries()) {
      //NOLOG               s.add(b.getKey() + " ");
      //NOLOG           }
      //NOLOG           s.add("}\n");
      //NOLOG       }
      //NOLOG       if (recvs.size() > 0) {
      //NOLOG           s.add("  countReceives:{");
      //NOLOG           for (r in recvs.entries()) {
      //NOLOG               s.add("<id="+r.getKey().id+",src="+r.getKey().src + ">, ");
      //NOLOG           }
      //NOLOG           s.add("}\n");
      //NOLOG       }
      //NOLOG   }
      //NOLOG       }
      //NOLOG debug(s.toString());
      //NOLOG }
        return resolveReqs;
    }
