...
//FIXME: IMPORTANT: multiple remotes per Id for different src types, how set the source kind which we need in notifyParent?
//FIXME: how to know the 'kind' of my relation to parent, ASYNC or AT. if AT don't notifyParent()
//FIXME: countReceived must consider the kind
/**
...
class FinishResilientOptimistic extends FinishResilient implements CustomSerialization {
    ...
    public static final class OptimisticMasterState extends FinishMasterState implements x10.io.Unserializable {
        ...
        def this(_id:Id, _parentId:Id, _numActive:Long, _transit:HashMap[Edge,Int],
                    ...
                    if (t1 >= t2) {
                        ...
                        if (key.kind == ASYNC) {
	                        for (1..count) {
	                            ...
	                            val dpe = new DeadPlaceException(Place(key.dst));
        ...
        public def convertFromDead(newDead:HashSet[Int], countReceived:HashMap[FinishResilientOptimistic.ReceivedQueryId, Int]) {
            ...
            for (e in transit.entries()) {
                ...
                if (newDead.contains(key.src)) {
                    ...
                    assert t1 > 0 : here + " Master(id="+id+").convertFromDead FATAL error, t1 must be positive";
                    if (t1 >= t2) {
                        val count = t1-t2;
                        val oldActive = numActive;
                        numActive -= count;
                        
                        if (verbose>=1) debug("==== Master(id="+id+").convertFromDead t1["+t1+"] t2["+t2+"] numActive["+oldActive+"] changing numActive to " + numActive);
                    ...
                    else assert false: here + " Master(id="+id+").convertFromDead FATAL error, t1 >= t2 condition not met";
                }
            }
            for (e in toRemove)
                transit.remove(e);
       ...
       def localFinishExceptionPushed(t:CheckedThrowable) {
        ...
        def transitToCompleted(srcId:Long, dstId:Long, kind:Int, t:CheckedThrowable, tag:String, resp:MasterResponse) {
            try {
                ...
                if (verbose>=3) {
                    debug("==== Master(id="+id+").transitToCompleted srcId=" + srcId + ", dstId=" + dstId + " dumping after update");
                    dump();
                }
        ...
        def notifyParent() {
            ...
            Runtime.submitUncounted( ()=>{
                ...
                val req = FinishRequest.makeTermRequest(parentId, FinishResilient.UNASSIGNED, DUMMY_INT, DUMMY_INT, srcId, dstId, optKind);
                req.ex = dpe;
                val resp = FinishReplicator.exec(req);
        ...
        def notifySubActivitySpawn(dstPlace:Place, kind:Int):void {
            ...
            if (srcId == dstId) {
            	...
            	val req = FinishRequest.makeTransitRequest(id, parentId, optSrc.id as Int, optKind, srcId, dstId, kind);
            	FinishReplicator.exec(req);
        ...
        def notifyActivityCreationFailed(srcPlace:Place, t:CheckedThrowable, kind:Int):void { 
            ...
            Runtime.submitUncounted( ()=>{
                ...
                val req = FinishRequest.makeTermRequest(id, parentId, optSrc.id as Int, optKind, srcId, dstId, kind);
                req.ex = t;
                val resp = FinishReplicator.exec(req);
        ...
        def pushException(t:CheckedThrowable):void {
            ...
            val req = FinishRequest.makeExcpRequest(id, parentId, optSrc.id as Int, optKind, t);
            val resp = FinishReplicator.exec(req);
            if (verbose>=1) debug("<<<< Root(id="+id+").pushException(t="+t.getMessage()+") returning");
        }
    ...
    public static final class OptimisticBackupState extends FinishBackupState implements x10.io.Unserializable {
        ...
        def this(id:FinishResilient.Id, _parentId:FinishResilient.Id, src:Place, kind:Int, _numActive:Long, _transit:HashMap[FinishResilient.Edge,Int],
            ...
            for (e in transit.entries()) {
                ...
                if (newDead.contains(key.dst)) {
                    ...
                    if (t1 >= t2) {
                        ...
                        val oldActive = numActive;
                        numActive -= count;
                        if (verbose>=1) debug("==== Backup(id="+id+").convertToDead t1["+t1+"] t2["+t2+"] numActive["+oldActive+"] changing numActive to " + numActive);
                        if (t2 == 0n)
                            toRemove.add(key);
                        else
                            transit.put(key, t2);
        ...
        public def convertFromDead(newDead:HashSet[Int], countReceived:HashMap[FinishResilientOptimistic.ReceivedQueryId, Int]) {
            ...
            for (e in transit.entries()) {
                ...
                if (newDead.contains(key.src)) {
                    ...
                    val rcvId = FinishResilientOptimistic.ReceivedQueryId(id, key.src, key.dst);
                    val t2 = countReceived.get(rcvId);
                    assert t1 > 0 : here + " Backup(id="+id+").convertFromDead FATAL error, t1 must be positive";
                    if (t1 >= t2) {
                        val count = t1-t2;
                        val oldActive = numActive;
                        numActive -= count;
                        if (verbose>=1) debug("==== Backup(id="+id+").convertFromDead t1["+t1+"] t2["+t2+"] numActive["+oldActive+"] changing numActive to " + numActive);
                        if (t2 == 0n)
                            toRemove.add(key);
                        else
                            transit.put(key, t2);
                        assert numActive >= 0 : here + " Backup(id="+id+").convertFromDead FATAL error, numActive must not be negative";
                        // we don't add DPE when src is dead; we can assume that the message was not sent as long as it was not received.
                    }
                    else assert false: here + " Backup(id="+id+").convertFromDead FATAL error, t1 >= t2 condition not met";
                }
            }
            for (e in toRemove)
                transit.remove(e);
        ...
        public def setLocalTaskCount(localChildrenCount:Int) {
            if (verbose>=1) debug(">>>> Backup(id="+id+").setLocalTaskCount called, localChildrenCount = " + localChildrenCount);
        ...
        def inTransit(srcId:Long, dstId:Long, kind:Int, tag:String) {
        	try {
                ...
                if (verbose>=3) {
                    debug("==== Backup(id="+id+").inTransit (srcId=" + srcId + ", dstId=" + dstId + ") dumping after update");
                    dump();
                }
        ...
        def transitToCompleted(srcId:Long, dstId:Long, kind:Int, ex:CheckedThrowable, tag:String) {
            try {
                ...
                numActive--;
                if (verbose>=3) {
                    debug("==== Backup(id="+id+").transitToCompleted (srcId=" + srcId + ", dstId=" + dstId + ") dumping after update");
                    dump();
                }
        ...
        def transitToCompletedUnsafe(srcId:Long, dstId:Long, kind:Int, cnt:Int, tag:String) {
            try {
                ...
                numActive-=cnt;
                if (verbose>=3) {
                    debug("==== Backup(id="+id+").transitToCompletedMul (srcId=" + srcId + ", dstId=" + dstId + ") dumping after update");
                    dump();
                }
                if (quiescent()) {
                    isReleased = true;
        ...
        def notifyParent() {
            ...
            if (optKind == FinishResilient.ASYNC) {
            	...
            	dpe = null;
            Runtime.submitUncounted( ()=>{
                if (verbose>=1) debug(">>>> Backup(id="+id+").notifyParent(srcId=" + srcId + ",dstId="+dstId+",kind="+optKind+") called");
                val req = FinishRequest.makeTermRequest(parentId, FinishResilient.UNASSIGNED, DUMMY_INT, DUMMY_INT, srcId, dstId, optKind);
                req.ex = dpe;
                val resp = FinishReplicator.exec(req);
                if (verbose>=1) debug("<<<< Backup(id="+id+").notifyParent(srcId=" + srcId + ",dstId="+dstId+",kind="+optKind+") returning");
            });
        }
            ...
            for (bx in backups) {
                ...
                if (master.isDead()) {
                    ...
                    at (master) @Immediate("create_masters") async {
                        val newM = new OptimisticMasterState(_id, _parentId, _numActive, _transit,
    ...
    static def notifyPlaceDeath():void {
        ...
        else if (verbose>=1) debug("==== createOrSyncBackups bypassed ====");
        ...
        for (b in backups) {
            ...
            backup.unlock(); //as long as we keep placeOfMaster = -1n no changes will be permitted on backup
        }
        ...
        //TODO: test cases when master or backup quiecent
...
/*
class MasterSyncParameters {
    val id:ResilientFinish.Id;
    val parentId:ResilientFinish.Id;
    val optSrc:Place;
    val numActive:Long;
    val transit:HashMap[FinishResilient.Edge,Int];
    var excs:GrowableRail[CheckedThrowable];

    public def this(m:OptimisticMasterState) {
        id = m.id;
        parentId = m.parentId;
        src = m.optSrc;
        numActive = m.numActive;
        transit = new HashMap[FinishResilient.Edge,Int]();
        for (e in transit.entries()) {
            transit.put(e.getKey(), e.getValue());
        }
        excs = new GrowableRail[CheckedThrowable](m.excs.size);
        for (e in m.excs)
            excs.add(e);
    }
}*/