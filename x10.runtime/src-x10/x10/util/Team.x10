...
*/
    ...
    private static class LocalTeamState(places:PlaceGroup, teamid:Int, myIndex:Long) {
        ...
        private def collective_impl[T](collType:Int, root:Place, src:Rail[T], src_off:Long, dst:Rail[T], dst_off:Long, count:Long, operation:Int, offsets:Rail[Int], counts:Rail[Int]):void {
                        ...
                        if (x10.xrx.Runtime.RESILIENT_MODE > 0 && dummyAtCount == DUMMY_AT_COUNT) {
                            ...
                            if (DEBUGINTERNALS && here.id == 0){
                                ...
                                if (Team.state(teamidcopy).local_child2Index > -1){
                                    Console.OUT.println("child2 is ["+((Team.state(teamidcopy).local_child2Index))+"] dead:" + Team.state(teamidcopy).places(Team.state(teamidcopy).local_child2Index).isDead());
                                }
                            }
                            if (FORCE_PROBE)
            ...
            //    Console.OUT.println("["+here+"] leaving the collective early, it is invalid (1) ...");
