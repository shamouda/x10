...
public class LogisticRegressionResilient implements LocalViewResilientIterativeAppOpt {
    ...
    public static def make(mX:Long, nX:Long, nRowBs:Long, nColBs:Long, nzd:Float, it:Long, nit:Long, chkpntIter:Long, places:PlaceGroup, team:Team){
        ...
        return new LogisticRegressionResilient(X, y, w, logisticD, it, nit, nzd, chkpntIter, places, team);
    }
    ...
    public def run(startTime:Long) {
        ...
        val places = X.places();
        appTempDataPLH = PlaceLocalHandle.make[AppTempData](places, ()=>new AppTempData(0, 0));
    ...
    public def checkpoint_local(store:DistObjectSnapshot):void {
    	finish {
    	    if (appTempDataPLH().iter == 0) {  
    	    	 async X.makeSnapshot_local(store);
    	    }
    	    async y.makeSnapshot_local(store);
    	    async grad.makeSnapshot_local(store);
    	    async o.makeSnapshot_local(store);
    	    async w.makeSnapshot_local(store);
            async logisticD.makeSnapshot_local(store);
            
            appTempDataPLH().checkpointObj=appTempDataPLH().obj;
            appTempDataPLH().checkpointDelta=appTempDataPLH().delta;
    	}
    }
    
    public def restore_local(store:DistObjectSnapshot, lastCheckpointIter:Long):void {
    	finish {
    	    async X.restoreSnapshot_local(store);
    	    async y.restoreSnapshot_local(store);
    	    async grad.restoreSnapshot_local(store);
    	    async o.restoreSnapshot_local(store);
    	    async w.restoreSnapshot_local(store);
            async logisticD.restoreSnapshot_local(store);
            
    	    appTempDataPLH().iter = lastCheckpointIter;
    	    appTempDataPLH().delta = appTempDataPLH().checkpointDelta;
    	    appTempDataPLH().obj = appTempDataPLH().checkpointObj;
    	}
    }
    ...
    public def remake(newPlaces:PlaceGroup, newTeam:Team, newAddedPlaces:ArrayList[Place]):void{
        ...
        val checkpointDelta = appTempDataPLH().delta;
        val checkpointObj = appTempDataPLH().obj;
        for (sparePlace in newAddedPlaces){
            Console.OUT.println("Adding place["+sparePlace+"] to appTempDataPLH ...");
            PlaceLocalHandle.addPlace[AppTempData](appTempDataPLH, sparePlace, ()=>new AppTempData(checkpointObj, checkpointDelta));
        }
    ...
    class AppTempData{
        ...
        public var checkpointObj:ElemType;
        public var checkpointDelta:ElemType;
    
        def this(checkpointObj:ElemType, checkpointDelta:ElemType){
        	this.checkpointObj = checkpointObj;
        	this.checkpointDelta = checkpointDelta;
        }
