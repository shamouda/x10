...
public class RunPageRank {
    public static def main(args:Rail[String]): void {
	    ...
	    if ((mG<=0) || nonzeroDensity <= 0.0 || sparePlaces < 0 || sparePlaces >= Place.numPlaces())
            ...
            val resilientStore = ResilientStore.make(sparePlaces);
            
            val places = resilientStore.getActivePlaces();
            val rowBlocks = opts("r", places.size());
            val colBlocks = opts("c", 1);

            val paraPR = PageRank.make(mG, nonzeroDensity, iterations, tolerance, rowBlocks, colBlocks, checkpointFreq, places, resilientStore);
