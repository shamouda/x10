...
public class PageRank implements SPMDResilientIterativeApp {
    ...
    public static def make(gN:Long, nzd:Float, it:Long, tolerance:Float, numRowBs:Long, numColBs:Long, chkpntIter:Long, places:PlaceGroup, resilientStore:ResilientStore) {
    ...
    private def initRandom(nzd:Float):void {
         val places = resilientStore.getActivePlaces();
    ...
    public def getCheckpointData_local():HashMap[String,Cloneable] {
    	...
    	if (appTempDataPLH().iter == 0) {
    		map.put("G", G.getCheckpoint_local());
    	}
    	//map.put("U", U.getCheckpoint_local());
    	map.put("P", P.getCheckpoint_local());
    	map.put("app", appTempDataPLH());
    ...
    public def restore_local(restoreDataMap:HashMap[String,Cloneable], lastCheckpointIter:Long) {
    	G.restore_local(restoreDataMap.getOrThrow("G"));
    	//U.restore_local(restoreDataMap.getOrThrow("U"));
    	P.restore_local(restoreDataMap.getOrThrow("P"));
    	appTempDataPLH().reinitialize(restoreDataMap.getOrThrow("app"));
    ...
    class AppTempData implements Cloneable {
        ...
        public def reinitialize(o:Cloneable) {
