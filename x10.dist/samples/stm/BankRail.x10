...
public class BankRail(places:Long, accountsPerPlace:Long, transfersPerPlace:Long, debugProgress:Long) 
    ...
    public static def main(args:Rail[String]) {
        ...
        val app = new BankRail(activePlaces.size(), accountsPerPlace, transfersPerPlace, debugProgress);
        val executor = MasterWorkerExecutor.makeRail(activePlaces, app, accountsPerPlace, (i:Long)=> {return 0;});
        try {
    ...
    public def execWorker(vid:Long, store:TxStore, recovery:Boolean):Any {
        ...
        for (i in 1..transfersPerPlace) {
            ...
            val distClosure = (tx:Tx) => {
                tx.asyncAt(p1, () => {
                    val oldV = tx.getRail(randAcc1) as Long;
                    tx.putRail(randAcc1, oldV - amount);
                    tx.asyncAt(p2, () => {
                        val oldV2 = tx.getRail(randAcc2) as Long;
                        tx.putRail(randAcc2, oldV2 + amount);
                    });
                });
            };
