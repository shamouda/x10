...
public final class ClusteringRail(plh:PlaceLocalHandle[ClusteringState]) implements MasterWorkerApp {
    ...
    private def createCluster(store:TxStore, tx:Tx, root:Int, placeId:Long, clusterId:Long, 
        ...
        val color = tx.getRail(root as Long) as Long;
        if (color == NO_COLOR) {
            //if the root is not taken, take it
            tx.putRail(root as Long, getColor(placeId, clusterId));
            ...
            for (var tmpP:Long = 0; tmpP < places; tmpP++) {
                val localMap = at (activePlaces(tmpP)) {
                    ...
                    while (setIter.hasNext()) {
                        ...
                        val clusterId = tx.getRail(vertex as Long) as Long;
                        if (clusterId != 0) {
...
class ClusteringState(N:Int) {
    ...
    public def this(graph:Graph, places:Long, workersPerPlace:Long, verticesPerPlace:Long,
        ...
        this.verticesPerPlace = verticesPerPlace;
